...
export const updateProduct = (formData) =>
    API.patch(`/products/${formData.productId}`, formData);
...
export const getUserBySearch = (email, name) =>
    API.get(`/users/search?email=${email}&name=${name}`);
export const updateUser = (formData) =>
    API.patch(`/users/${formData.userId}`, formData);
...
export const getOrderBillingDetails = (orderId) =>
    API.get(`/orders/billings/${orderId}`);
export const getOrderSummary = () => API.get("/orders/summary");
export const updateOrder = (orderId, formData) =>
    API.patch(`/orders/${orderId}`, formData);
